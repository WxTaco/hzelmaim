...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let pg_auth_store: Arc<dyn hzel_backend::auth::store::AuthStore> =
        Arc::new(PgAuthStore::new(pool.clone()));
    ...
    if config.bootstrap_dev_session {
        ...
        if let Some((existing_id,)) = row {
            ...
            in_memory_store
                .patch_user_id(seeded.user.id, existing_id)
                .await;
    ...
    // Repositories
    let audit_repo: Arc<dyn hzel_backend::db::audit_repo::AuditRepo> =
        Arc::new(PgAuditRepo::new(pool.clone()));
    let container_repo: Arc<dyn hzel_backend::db::container_repo::ContainerRepo> =
        Arc::new(PgContainerRepo::new(pool.clone()));
    let command_repo: Arc<dyn hzel_backend::db::command_repo::CommandRepo> =
    ...
    let proxmox: Arc<dyn hzel_backend::proxmox::client::ProxmoxClient> = if config
    ...
    let _sync_handle =
        state_sync::spawn_state_sync(container_repo, proxmox, Duration::from_secs(30));
